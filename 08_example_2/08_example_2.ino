...
float dist_min, dist_max, dist_mid, dist_raw, dist_temp; // unit: mm
...
// get a distance reading from the USS
  dist_temp = USS_measure(PIN_TRIG,PIN_ECHO);
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if(dist_raw == dist_mid){
      analogWrite(PIN_LED, 0);
    }else if(dist_raw < dist_mid){
      analogWrite(PIN_LED, int((dist_mid - dist_raw)/100 * 255));
    }else {
      analogWrite(PIN_LED, int((dist_raw - dist_mid)/100 * 255));
    }
  }
